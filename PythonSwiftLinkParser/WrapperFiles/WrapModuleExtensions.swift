...
//                    """)
            ...
            protocol \(cls.title)_Delegate {
                \(if: cls.callbacks_count > 0, "func set_\(cls.title)_Callback(callback: \(cls.title)PyCallback\(if: !cls.singleton, ", cython_class: CythonClass"))")
        ...
        for cls in classes {
            ...
            let set_call_pointers = functions.compactMap{ f->String? in
                ...
                if let call_target = f.call_target {
                    return "\(if: f.options.contains(.direct), "", "_")\(f.name) = pycall.\(call_target).\(f.name)"
                }
                
                return "\(if: f.options.contains(.direct), "", "_")\(f.name) = pycall.\(f.name)"
                
            }.joined(separator: newLineTabTab)
            let call_funcs = functions.filter(
                {!$0.options.contains(.direct)}).map{ f -> String in
                    let _args_ = f.args.filter{ a -> Bool in
                        if f.call_class_is_arg { if a.name == f.call_class! { return false } }
                        if f.call_target_is_arg { if a.name == f.call_target! { return false } }
                        return true
                    }.map{a -> String in
                        if a.asObjectEnum != nil { return "\(a.name).rawValue" }
                        if a.has_option(.dispatch) { return "\(a.name).rawValue" }
...
//                    }.compactMap({$0})
                    ...
                    return """
                        ...
                        @inlinable
                        func \(f.name)(\(f.export(options: [.use_names, .swift, .callback])))\(if: use_rtn," -> \(f._return_.swift_callback_return_type)","") {
...
//                    }.joined(separator: ", ")
                    ...
                    var func_string = "\(return_type2) \(cls.title)_\(function.name)(\(send_args))"
